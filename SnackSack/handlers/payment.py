...
class Product:
    def __init__(self, title: str, description: str, label: str, price: int):
        ...
        self.currency = "rub"
        self.prices = [LabeledPrice(label=label, amount=(price * 100))]
...
async def shipping_process(shipping_query: ShippingQuery):
    if shipping_query.shipping_address.country_code != "RU":
        return await bot.answer_shipping_query(
            shipping_query.id, ok=False, error_message="Россию там выбрать надо да."
        )

    pickup = ShippingOption(id="pickup", title="Самовывоз")
